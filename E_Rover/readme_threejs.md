# ESP32-ROVER Threejs installation

To view 3D motion in javascript browser interface, E_rover uses webGL primitives from threejs libraries.

Because these libraries are too larges to be stored in esp32 data folder, they are directly installed on the smartphone (they could be load from internet but internet is not always avalaible & CORS problems appear). To do this, a local node server is installed on the smartphone to serve the javascript.

# User installation

  * Copy ROVER.JS folder at the root of the smartphone SD card
  * Install [Termux](https://weekly-geekly-es.imtqy.com/articles/fr444950/index.html) which is a chroot linux running in Android environnement
  * Install openssh server 
  * Install [scrcpy](https://smart-tech.mg/comment-controler-et-mettre-en-miroir-un-telephone-android-dans-ubuntu/) to keep control of smartphone screen on ubuntu desktop.


  * Install npm & node
  * Install Threejs & OrbitControls
...
# Developper installation
  ...
  * Install [Termux](https://weekly-geekly-es.imtqy.com/articles/fr444950/index.html) which is a chroot linux running in Android environnement
  * Using SSH
  * * apt install openssh <br> ggffg
  * * whoami # identify your {user} name
  * * passwd # define your password
  * * ifconfig #identify the {IP} address
...
## Table of contents
- [USER quick start ](#espasyncwebserver-)

As shown html motion interface presents parameters from deault configuration. You have to consider different restrictions

- MOTION toogle actives motion detection and displays currents parameters
    - Grid step defines the sub-sampling of the camera frames. For example, a step of 4 will calculate the detection on 4x4 areas, ie 16 pixels, which represents 19200 areas for an image of 640x480 resolution. 19200 is the maximum number of zones allowed. For higher resolution images increase the step.  
    - Absolute threshold measures the absolute variation value between the pixels of the current camera frame and those of the smoothed preceding ones. 

    - % Relative threshold measures the variation in percents between the pixels of the current camera frame and those of the smoothed preceding ones. Keep in mind that this criterion will only work correctly on images without very dark areas. Indeed because of moise, an almost black pixel can see its value go easily from 1 or 5 (that is to say 500% of variation) from where a possibly unjustified alert.


    - % Smoothing factor If value is 0.0 previous frame is strictly the previous one from the camera, if value is 1.0 the previous never change as a freezed reference frame.
    - Alert pixels count
    - % Alert pixels ratio
    - Alert FTP
    - Alert SMS
...
When program is uploaded & configuration files installed on SD card ESP32-CAM runs. 

    - [Configuration files:]
    - 
    - 

- [HARDWARE ](#espasyncwebserver-)

- [SOFWARE ](#espasyncwebserver-)

- [TOOLS ](#espasyncwebserver-)
    - [Encode html source](https://gchq.github.io/CyberChef/#recipe=Gzip&#40;'Dynamic%20Huffman%20Coding','','',false&#41;To_Hex&#40;'0x',0&#41;Split&#40;'0x',',%200x'&#41;&input=...)
    - [Decode gzip html](https://gchq.github.io/CyberChef/#recipe=Find_/_Replace&#40;%7B'option':'Regex','string':','%7D,'',true,false,false,false;&#41;Remove_whitespace&#40;true,false,true,true,false,false;&#41;From_Hex&#40;'0x';&#41;Gunzip&#40;&#41;)
    - 
    - 

- [REFERENCES ](#espasyncwebserver-)
...
To use this <u>library you might</u> need to have the latest git versions of [ESP8266](https://github.com/esp8266/Arduino) Arduino Core
