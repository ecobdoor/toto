...
Slot_pilot = function(MODULES, MID, DBG_loc, DBG_rov, EXE, TIM, TIO, JOYSTICK,
	...
	this.update2reply = function(KMD) {
		...
		(0/1) .........0 rover(1) <= REPLY DRV [rQp~DRV≈>DRV\Pilot Infos{"CTL":{"TYP":"rRp","TIK":0},
		"KMD":{"MID":"DRV","DT":103,"OP":"Inf",
		"S":0,"D":0,"L":0,"R":0,"X":true,
		"MTS":440460769,"DST":0.31,"DTK":0,
		"XC0":-461.7225,"YC0":-216.3388,"ZC0":-29.06748,
		"SPX":-2165.875,"SPY":-898.0837,"SPZ":-132.7734,
		"DAX":-0.917501,"DAY":-4.372124,"DAZ":-0.156374,
		"DNR":-1.247242,"DEP":-3.965219,"DDY":61.25501,
		"DGX":69.1509,"DGY":21.71465,"DGZ":997.3699,
		"Iax":-0.070068,"Iay":0.017343,"Iaz":0.997214,
		"Igx":0.104925,"Igy":0.223382,"Igz":0.153562,
		"Imx":40.56612,"Imy":132.7552,"Imz":145.2944}}] delta(TIK=147)
		*/
...
function _CanvasUmi(CANVAS, YMIN, YMAX, GAP, TITLE, LEGENDS_COLORS/* LEGEND_A,COLOR_A,LEGEND_B,COLOR_B */) {
	...
	this.Init = function() {
		...
		this._CTX.fillText(SF_float(YMIN, 7, 1), 10 + s, col(YMIN) - s);
		this._CTX.fillText('+' + SF_float(YMAX, 7, 1), 10 + s, col(YMAX) + s);
	...
	this.addValue = function(Y, COLOR) {
		...
		this.Point(Gleft + SELF.Gwidth, col(Y));
	}
	// -------------------------------------------------------
...
function _Canvas2DUmi(CANVAS, XMIN, XORG, XMAX, YMIN, YORG, YMAX, LFT, RGT, TOP, BOT,
	...
	this.Init2D = function() {
		...
		*/
		this._CTX.clearRect(- LFT, - TOP, LFT + RGT + this._CANVAS.width, TOP + BOT + this._CANVAS.height);
		this._CTX.fillStyle = "#008800";
		this._CTX.fillRect(- LFT, - TOP, LFT + RGT + this._CANVAS.width, TOP + BOT + this._CANVAS.height);
		this._CTX.fillStyle = "#ffffff";
		this._CTX.fillRect(0, 0, this.Gwidth, this.Gheight);
		// DRAW THE FRAME
		...
		// Draw dark red borders
		{
			this._CTX.strokeStyle = '#880000';
			var s = 2;
			this._CTX.moveTo(- s, - s);// On se déplace au coin inférieur gauche
			this._CTX.lineTo(- s, + this.Gheight + s);
			this._CTX.lineTo(+ this.Gwidth + s, + this.Gheight + s);
			this._CTX.lineTo(+ this.Gwidth + s, - s);
			this._CTX.lineTo(- s, - s);
			this._CTX.stroke();// On trace seulement les lignes.
		}
		// Draw legends & untag the old crosses
		{
			this._CTX.font = "15px Arial";
			this._CTX.fillText(TITLE, 10, -6);
			for (var i = 0; i < LEGENDS_COLORS.length; i++) {
				this._CTX.fillStyle = LEGENDS_COLORS[i].COLOR;
				this._CTX.fillText(LEGENDS_COLORS[i].LEGEND, 100 + 50 * i, -6);
				LEGENDS_COLORS[i].oldCross = false;
			};
		}
		// draw origin axis
		{
			this.Line(col(XMIN), row(YORG), col(XMAX), row(YORG), "#333333");
			this.Line(col(XORG), row(YMIN), col(XORG), row(YMAX), "#ff00ff");
		}
		// text axis min max values
		{
			this._CTX.fillStyle = "#000000";
			this._CTX.font = "10px Arial";
			this._CTX.fillText(SF_float(XMIN, 7, 1), col(XMIN) - 10, row(YMIN) + 15);
			this._CTX.fillText(SF_float(XORG, 7, 1), col(XORG) - 20, row(YMIN) + 15);
			this._CTX.fillText(SF_float(XMAX, 7, 1), col(XMAX) - 30, row(YMIN) + 15);
			// preserve the transformation matrix!
			this._CTX.rotate(-Math.PI / 2);
			this._CTX.fillText(SF_float(YMIN, 7, 1), -row(YMIN) - 10, -5);
			this._CTX.fillText(SF_float(YORG, 7, 1), -row(YORG) - 20, -5);
			this._CTX.fillText(SF_float(YMAX, 7, 1), -row(YMAX) - 30, -5);
			this._CTX.rotate(Math.PI / 2);
		}
		// Just to play...
		{
			this._CTX.moveTo(-LFT + 10, -TOP);
			this._CTX.arc(-LFT, -TOP, 10, 0, Math.PI / 2); // On trace la courbe délimitant notre forme
			this._CTX.stroke();// On trace seulement les lignes.
		}
		this._CTX.closePath();
	}
	...
	//
	function col(X) {
		var abs = SELF.Gwidth - SELF.Gwidth * (XMAX - X) / (XMAX - XMIN);
		return abs;
	}
	// -------------------------------------------------------
	// row
	//
	function row(Y) {
		var ord = SELF.Gheight - SELF.Gheight * (Y - YMIN) / (YMAX - YMIN);
		return ord;
	}
	// -------------------------------------------------------
	// moveMark
	...
	this.moveMark = function(ID, X, Y) {
		...
		this.Point(Col, Row, LEGENDS_COLORS[ID].COLOR);
		if (LEGENDS_COLORS[ID].oldCross) {
			SELF._CTX.putImageData(LEGENDS_COLORS[ID].savedCross,
				LEGENDS_COLORS[ID].prvCol + LFT - Csiz, LEGENDS_COLORS[ID].prvRow + TOP - Csiz);
			SELF.Line(Col, Row, LEGENDS_COLORS[ID].prvCol, LEGENDS_COLORS[ID].prvRow,
				LEGENDS_COLORS[ID].COLOR);
		}
		LEGENDS_COLORS[ID].savedCross = SELF._CTX.getImageData(Col + LFT - Csiz, Row + TOP - Csiz,
			2 * Csiz + 1, 2 * Csiz + 1);
		LEGENDS_COLORS[ID].oldCross = true;
		LEGENDS_COLORS[ID].prvRow = Row;
		LEGENDS_COLORS[ID].prvCol = Col;

		// this._CTX.globalCompositeOperation = 'copy';
		this.Xcross(Col, Row, LEGENDS_COLORS[ID].COLOR);
	}
	SELF.Init2D();
}
///////////////////////////////////////////////////////////////////////
...
function showViewsDefault(areaID) {
	...
	/*
		for (var key in LAYOUT[areaID]) {
			LAYOUT[key].className = LAYOUT[key].className.replaceAll(" " + areaID + "_offset", "");
		}
	*/
}
...
function ROAM(THIS) {
	if (THIS.className === "bigBtn") {
		...
		//		setTimeout(stopJOYSTICK, 750);// to be sure rover pass in idle mode
	}
	_WSROV.sokROVER.sendCLI("RR");
}
//----------------------------------------------------------------------
...
function Start() {
	...
	var CANVAS_MAP = new _Canvas2DUmi('MAPcanvas', xMin, xOrg, xMax, yMin, yOrg, yMax, 20, 2, 25, 20, 'MAP',
		[{
			LEGEND: 'Acc',
			COLOR: "#0000ff"
		}, {
