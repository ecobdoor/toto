...
world_Matrix=new THREE.Matrix4();
		...
		// cameras
		cameras['GND'] = Camera('gnd', 150, 150, 150, 0, 0, 0,0xff0000);
		...
		*/
		...
		cameras['ROV'] = Camera('rov', 0, 0, 100, 180, 0, 0,0x0000ff);
		...
		*/
		...
		flagCam:false
			...
			*/
			...
			cameras['GND'][0].setRotationFromEuler(
				new THREE.Euler(
					THREE.MathUtils.degToRad(webglCTRL.myCamera.rollCam),
					THREE.MathUtils.degToRad(webglCTRL.myCamera.pitchCam),
					THREE.MathUtils.degToRad(webglCTRL.myCamera.yawCam)
					)
				)
			cameras['GND'][0].lookAt(_Rover._world);
				cameraControls = new OrbitControls(cameras['GND'][0], renderer.domElement);
			cameras['GND'][1].update();
			webglCTRL.myCamera.flagCam=false;
		}
		// cameras switch
		if (Kam_flag) {
			if (Kam_reset) {
