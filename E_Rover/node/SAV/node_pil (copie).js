...
*/
	this.reply2query = function(KMD) {
		//SELF.TIO._setText("DRIVER reply2query > [" + JSON.stringify(KMD) + "]");// for debug...
		if (KMD['endCalPt'] !== undefined) {
			IMU.message(KMD['CNT']+ ' pts handshacked');
			if((calMag_START)||(rawMag_Flag)||(rawMag_FlagShift)||(calMag_Flag))// if threejs has not finish
				KMD['endCalPt']=false;
			else
				KMD['endCalPt']=true;
			return KMD;
			...
			if (calMag_START){				alert('ERROR calMag_START is true !!!');				return;			}
			IMU.message(KMD['CNT']+ ' cal mag view inialized');
			KMD['endCalPt']=false;
			calMag_START = true;
			return KMD;
		}
		else if (KMD["OP"] === "calMag_STOP") {
			IMU.message(KMD['MSG'],5000);
			alert(KMD['MSG']);
			enableChildren('IMU_head');
			KMD['endCalPt']=true; // pour end !!!!
			return KMD;
		}
		else if (KMD["OP"] === "rawMAGpoint") {
			if (rawMag_Flag){				alert('ERROR rawMag_Flag is true !!!');				return;			}
			rawMag_Data[rawMag_ptr++] = KMD[DEF_KF_Imx];
			rawMag_Data[rawMag_ptr++] = KMD[DEF_KF_Imy];
			rawMag_Data[rawMag_ptr++] = KMD[DEF_KF_Imz];
			rawMag_Count++;
			IMU.message(KMD['CNT']+ ' pts acquired');
			rawMag_Flag = true;
		}
		else if (KMD["OP"] === "rawMAGshift") {
			if (rawMag_FlagShift){			alert('ERROR rawMag_FlagShift is true !!!');		return;			}
			rawMag_Data[rawMag_ptr++] = KMD[DEF_KF_Imx];
			rawMag_Data[rawMag_ptr++] = KMD[DEF_KF_Imy];
			rawMag_Data[rawMag_ptr++] = KMD[DEF_KF_Imz];
			rawMag_ptr-=3;
		...
		else if (KMD["OP"] === "calMAGpoint") {
			if (calMag_Flag){				alert('ERROR calMag_Flag is true !!!');				return;			}
			calMag_Data[calMag_ptr++] = KMD[DEF_KF_Imx];
			calMag_Data[calMag_ptr++] = KMD[DEF_KF_Imy];
			calMag_Data[calMag_ptr++] = KMD[DEF_KF_Imz];
			calMag_Count++;
			IMU.message(KMD['CNT']+ ' pts verified');
			calMag_Flag = true;
		}
	...
	*/
	...
	this.update2reply = function(KMD) {
		if (KMD['OP'] === 'setPARAM') {
			...
			SELF.TIO._setText("STANDBY IMU DESACTIVEE...");
			CANVAS_IMU.Init2D();
			CANVAS_MAP.Init2D();
			return;
		}
	...
	this.updateAttitude = function(KMD) {
		...
		{
			traject_Data[traject_ptr++] = KMD[DEF_KF_XC0];
			traject_Data[traject_ptr++] = KMD[DEF_KF_YC0];
			traject_Data[traject_ptr++] = KMD[DEF_KF_ZC0];
			traject_ptr -= 3;
			ATTITUDE[traject_ptr++] = KMD[DEF_KF_ROLL];
			ATTITUDE[traject_ptr++] = KMD[DEF_KF_PITCH];
			ATTITUDE[traject_ptr++] = KMD[DEF_KF_YAW];
			traject_ptr -= 3;
			GRAVITY[traject_ptr++] = KMD[DEF_KF_GRX];
			GRAVITY[traject_ptr++] = KMD[DEF_KF_GRY];
			GRAVITY[traject_ptr++] = KMD[DEF_KF_GRZ];
			traject_ptr -= 3;
			MAGNORTH[traject_ptr++] = KMD[DEF_KF_Imx];
			MAGNORTH[traject_ptr++] = KMD[DEF_KF_Imy];
			MAGNORTH[traject_ptr++] = KMD[DEF_KF_Imz];
			...
			traject_Count++;
			document.getElementById("wglCount").innerHTML = traject_Count.toString();
			traject_Flag = true;
		}
		{
			imu_flag=true;
...
// ////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------
function showView(areaID, viewID, HIDEALL) {
	/** *****************************************************************
	 * @fn clearArea(AREA)
	 * @brief Deletes AREA classes from LAYOUT & adds the entry default class to each item 
	 */
	...
	if ((HIDEALL == true) || (typeof HIDEALL === 'undefined'))
		clearArea(areaID);
	if (viewID === undefined)
		return;
...
/**
 * @fn
 * @brief toggle the dialog & close the others (if any)
 * @param THIS this of the onclick event
 */
/*
function control(THIS) {
	var elt = LAY OUT['CENTER_ul'][THIS.name + '_ctrl'].ELT;
	// set OFF previous dialog & display it at default place
	for (const [dlg, obj] of Object.entries(LAY OUT['CENTER_ul']))
		if ((obj.ON) && (obj.ELT != elt)) {
			obj.BTN.className = obj.BTN.className.replace(" checked", "");
			obj.ELT.className = obj.ELT.className + " " + dlg + "_default"
			obj.ON = false;
		}
	// toggle THIS dialog
	if (THIS.className === "checkBtn little") {
...
//----------------------------------------------------------------------
/*
function showM3D(viewID) {
	showView('CENTER_ur', viewID);
	showView('HEAD', viewID + '_head', false);
	if (LAY OUT['CENTER_ul'][viewID + '_ctrl'].ON)
		showView('CENTER_ul', viewID + '_ctrl', false);
}
...
function jsonXHR(REQUEST) {
	...
	xhttp.open("GET", "http://192.168.4.1/" + REQUEST, false);// false is synchrone !!!
...
*/
	...
	var _DRIVER = new Slot_driver(MODULES, "DRV",
		...
		new __textarea("classTA_i_o", 'ATT', "IDtio_DRIVER", CFG.client.__field_txtarea),
	...
	LAYOUT = {
		...
		'CENTER_ul': {
			...
			DBG_ctrl: { ELT: document.getElementById("DBG_ctrl") }
		},
		"HEAD": {
			IOworkjob_container: { ELT: document.getElementById("IOworkjob_container") },
