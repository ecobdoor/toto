...
void onWSMessage(AsyncWebSocket *server, AsyncWebSocketClient *client,
	...
	if (type == WS_EVT_CONNECT) {
		...
		_SERIAL_0("\n%s", WWW(pwsx).c_str());
		if (pwsx != nullptr) {
			client->text("ROVER client " + String(client->id())
 ...
 * @brief Manages json messages received from web sockets @ref findFCT
 *
 *
 * @param this_DebugLVL
 */
void core_websock_MSG(int8_t &this_DebugLVL){
	/*
	 * ==========================================
 ...
 * @param this_DebugLVL
 */
void core_modules_MSG(int8_t &this_DebugLVL){
	/*
	 * ==========================================
...
void core_TASK(void *pvParameters){
	...
	for (;;) {
		//digitalWrite(red2PIN_modules, HIGH);
		core_websock_MSG(this_DebugLVL);
		core_modules_MSG(this_DebugLVL);
		vTaskDelay(10);
...
void wsnd_websock_MSG(int8_t &this_DebugLVL){
	...
	while (myWSOK.pull(&item)) {
		try {
			...
			charFree(item.outMSG); // Make an ack fifo to every time delete in the same task than new !!!!
			digitalWrite(whi0PIN_Wsnd, LOW);
			vTaskDelay(10); //think about webSocket server to send... (10ms => 100 Hz)
		} catch (EXCEPT const &e) {
			charFree(item.outMSG);
			FIX_ERROR();
		} catch (std::exception const &e) {
			Serial.printf("\n@@@@@@@@@@@@@@@@@@@@@ %s @@@@@@@@@@", e.what());
		}
...
void wsnd_TASK(void *pvParameters){
	...
	int8_t &this_DebugLVL = *(tskInfo.DebugLVL);
	for (;;) {
		wsnd_websock_MSG(this_DebugLVL);
...
// LangSTR =============================================================
	 ...
	 */
	_SERIAL_0("\nUTF8 support:");
	_SERIAL_0("\nLook correct:  {◉➊➋➌➍➎➏➐➑➒⓪➀➁➂➃➄➅➆➇➈↡↠↢↣⇫ε⇪∆∇≚∞⋀⚻▴⑬⋁ᗄᗐထܠݎݏ߇߈");
	_SERIAL_0("\nLook correct:  {🇼⟰⟱⚺ƧɅ±∇¤øƟɅɣʬΔΛϪᴓᴧᵛ†‼∆∩⌂▬►◄◊¥Vv□ΥѲ˅˄▲▼ꜛꜜ◊↑↓↕↨ˠØ}");
	_SERIAL_0("\nSeem hazardous:{⮛⮟⮝   ߇߈  ⮊}");
	/*
	 Serial.printf("\n   8               9               A               B");
