...
//		_SERIAL_0("\nAddress %20X, err(%i)'%s'", address, error, Wire.getErrorText(error));
		_SERIAL_0("\nAddress %20X, err(%i)", address, error);
		...
		if (error == 0) {
			...
			_SERIAL_0("\n\tUnknown error at address %02X !", address);
		}
	}
	address = 0x68;
	for (size_t i = 0; i < 12; i++) {
		...
		if (error) {
			...
			_SERIAL_0("\nAddress %20X, err(%i)", address, error);
			_SERIAL_0("\n\tI2C device found at address %02X !", address);
			writeByte(address, 0x6B, 0x80); // Set bit 7 to reset MPU9250
