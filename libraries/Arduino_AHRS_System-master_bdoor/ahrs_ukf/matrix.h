...
//     _outp.vPrintFull("CholeskyDec _outp");     while (!Serial.available());
            return _outp;
    }
    ...
    /* Printing function -------------------------------------------------------------------------------------------- */
    ...
    #elif (SYSTEM_IMPLEMENTATION == SYSTEM_IMPLEMENTATION_EMBEDDED_ARDUINO)
        void vPrint(const char * TITLE=nullptr) {
                Serial.printf("\n%s (r,c)=[%i,%i]",TITLE?TITLE:"",this->i16row,this->i16col);
                Serial.printf(" %i ",uxTaskGetStackHighWaterMark(NULL));
            char _bufSer[10];
            for (int16_t _i = 0; _i < this->i16row; _i++) {
                Serial.print("\n[ ");
                for (int16_t _j = 0; _j < this->i16col; _j++) {
        ...
        void vPrintFull(const char * TITLE=nullptr) {
                Serial.printf("\n%s (r,c)=[%i,%i]",TITLE?TITLE:"",this->i16row,this->i16col);
                Serial.printf(" %i ",uxTaskGetStackHighWaterMark(NULL));
            char _bufSer[32];
            for (int16_t _i = 0; _i < this->i16row; _i++) {
