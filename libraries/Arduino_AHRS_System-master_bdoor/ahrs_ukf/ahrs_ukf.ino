...
UKF *UKFptr = new UKF(quaternionData, UKF_PINIT, UKF_Rv, UKF_Rn, Main_bUpdateNonlinearX,
...
void setup(){
	...
	xTaskCreatePinnedToCore(&xxx, // Task function.
		"toto", // name of task.
		48000, // Stack size
		nullptr, // parameter of the task
		tskIDLE_PRIORITY + 5, //ESP_TASK_PRIO_MAX - 14, // priority of the task from (tskIDLE_PRIORITY + 1) to (configMAX_PRIORITIES -1 )
...
bool elasedTime(){
	static uint64_t oldTime = esp_timer_get_time();
	uint64_t nowTime = esp_timer_get_time();
	if ((nowTime - oldTime) >= 1000 * SS_DT_MILIS) {
		oldTime = nowTime;
		return true;
	}
	return false;
...
void PRINT_north(){
	Serial.print("\nNorth vector value:  ");
	snprintf(bufferTxSer, sizeof(bufferTxSer) - 1, "%12.6f %12.6f %12.6f", IMU_MAG_B0[0][0],
		IMU_MAG_B0[1][0], IMU_MAG_B0[2][0]);
	Serial.print(bufferTxSer);
}
//---------------------------------------------------------------------
void PRINT_iron(){
	Serial.print("\nHard iron bias value:");
	snprintf(bufferTxSer, sizeof(bufferTxSer) - 1, "%12.6f %12.6f %12.6f\r\n", HARD_IRON_BIAS[0][0],
		HARD_IRON_BIAS[1][0], HARD_IRON_BIAS[2][0]);
	Serial.print(bufferTxSer);
}
//---------------------------------------------------------------------
void CALC_attitude(float &r, float &p, float &y, const float q0, const float q1, const float q2,
	const float q3){
#define pi  3.141592653589793238462643383279502884f
	...
	r = -atan2f(a31, a33);
	p = +asinf(a32);
	y = -atan2f(a12, a22) + (1.13333 * pi / 180); //LMD; // Local MAgnetic Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
	if (y < 0)
		y += 2 * pi; // Ensure yaw stays between 0 and 360
}
//---------------------------------------------------------------------
void PRINT_opengl(const float Ax, const float Ay, const float Az, const float Gx, const float Gy,
	const float Gz, const float Bx, const float By, const float Bz, const float q0, const float q1,
	const float q2, const float q3){
	float r, p, y;
	CALC_attitude(r, p, y, q0, q1, q2, q3);
	Serial.printf("?  %12.5f  %12.5f  %12.5f  %12.5f\n", q0, q1, q2, q3);
	char c = '@';
	Serial.printf("%c 123456", c);
//		Serial.printf("%c %6llu", c, (Fusensor.microTS/* - initial_microTimeStamp*/) / 1000);
	Serial.printf("  %12.5f  %12.5f  %12.5f", Ax, Ay, Az);
	Serial.printf("  %12.5f  %12.5f  %12.5f", Gx, Gy, Gz); //°/s
	Serial.printf("  %10.3f  %10.3f  %10.3f", Bx, By, Bz);
	if (c == '@')
		Serial.printf("  %12.5f  %12.5f  %12.5f", r * 180. / pi, p * 180. / pi, y * 180. / pi);
	Serial.printf(" \n");
}
///////////////////////////////////////////////////////////////////////////////
...
void xxx(void *pvParameters){
	for (;;) {
		if (elasedTime()) { /* We running the RLS/UKF at sampling time = 20 ms */
			/* Read the raw data */
			IMU.readSensor();
...
//			Serial.printf("\n> %10.3f | %10.3f %10.3f %10.3f | %10.3f %10.3f %10.3f | %10.3f %10.3f %10.3f",esp_timer_get_time() / 1000000., Ax, Ay, Az, Bx, By, Bz, p, q, r);
			//HARD_IRON_BIAS.vPrintFull();
			if (STATE_AHRS == STATE_UKF_RUNNING) { /* Run the UKF algorithm */
				//Serial.printf(" STATE_UKF_RUNNING");
				...
				float_prec _normG = sqrt(Y[0][0] * Y[0][0]) + (Y[1][0] * Y[1][0])
					+ (Y[2][0] * Y[2][0]);
				Y[0][0] = Y[0][0] / _normG;
				Y[1][0] = Y[1][0] / _normG;
				Y[2][0] = Y[2][0] / _normG;
				float_prec _normM = sqrt(Y[3][0] * Y[3][0]) + (Y[4][0] * Y[4][0])
					+ (Y[5][0] * Y[5][0]);
				Y[3][0] = Y[3][0] / _normM;
				Y[4][0] = Y[4][0] / _normM;
				Y[5][0] = Y[5][0] / _normM;
...
//     Y.vPrintFull("Y Acc 1:3 Mag 4:6");
//     U.vPrintFull("U Gyr raw data");
				/* ============================= Update the Kalman Filter ============================== */
				u64compuTime = esp_timer_get_time();
...
//					Serial.printf("\n< %10.3f | %10.3f %10.3f %10.3f | %10.3f %10.3f %10.3f |" %10.3f %10.3f %10.3f, esp_timer_get_time() / 1000000., Y[0][0], U[0][0], U[1][0], U[2][0]);
				}
//				Serial.printf("\n");
				u64compuTime = (esp_timer_get_time() - u64compuTime);
				/* ----------------------------- Update the Kalman Filter ------------------------------ */
						quaternionData = UKF_IMU.GetX();
				PRINT_opengl(Ax, Ay, Az, p, q, r, Bx, By, Bz, quaternionData[0][0],
					quaternionData[1][0], quaternionData[2][0], quaternionData[3][0]);

			} // end STATE_AHRS == STATE_UKF_RUNNING
				...
				RLS_gain = RLS_P * RLS_in
					/ (RLS_lambda + RLS_in.Transpose() * RLS_P * RLS_in)[0][0];
				...
				if ((P_check.Transpose() * P_check)[0][0] < 1e-4) {
					...
					Serial.printf("\n< Calibration finished, the hard-iron bias identified:");
					snprintf(bufferTxSer, sizeof(bufferTxSer) - 1, "%12.6f %12.6f %12.6f\r\n",
						HARD_IRON_BIAS[0][0], HARD_IRON_BIAS[1][0], HARD_IRON_BIAS[2][0]);
					Serial.println(bufferTxSer);
					Serial.println(
						"Set the X axis facing north *with z-accelerometer pointed down*, then send command 'f' to finished, 'a' to abort");
				}
				...
				if ((RLS_u32iterData % 100) == 0) {
					...
					Serial.printf(
						"%4i Calibration in progress, the hard-iron bias identified so far:",
						RLS_u32iterData);
					snprintf(bufferTxSer, sizeof(bufferTxSer) - 1, "%.3f %.3f %.3f (P = %f)\r\n",
						RLS_theta[0][0] / 2.0, RLS_theta[1][0] / 2.0, RLS_theta[2][0] / 2.0,
						(P_check.Transpose() * P_check)[0][0]);
					Serial.println(bufferTxSer);
				}
			...
			} // end STATE_AHRS == STATE_MAGNETO_BIAS_IDENTIFICATION
			...
			if (Serial.available()) {
				...
				if (STATE_AHRS == STATE_UKF_RUNNING) {
					...
					if (cmd == 'c') {
						...
						Serial.print("\nNorth vector identification command entered");
						Serial.print(
							"\nSet the X axis facing north *with z-accelerometer pointed down*, then send command 'f'");
						STATE_AHRS = STATE_NORTH_VECTOR_IDENTIFICATION;
					}
					...
					if (cmd == '?') {
						snprintf(bufferTxSer, sizeof(bufferTxSer) - 1, "UKF in Teensy 4.0 (%s)",
							(FPU_PRECISION == PRECISION_SINGLE) ? "Float32" : "Double64");
						Serial.printf("\n%s", bufferTxSer);
						Serial.printf(". Stack left %i bytes", uxTaskGetStackHighWaterMark(NULL));
						...
						while (!Serial.available())
							;
						uint8_t count = Serial.read() - 48;
						for (uint8_t _i = 0; _i < count; _i++) {
							Serial.printf("\n%4i Quat %10.3f %10.3f %10.3f %10.3f | %10.3f s",
								_i, quaternionData[0][0], quaternionData[1][0],
								quaternionData[2][0], quaternionData[3][0], u64compuTime / 1000.);
							/*
					...
					if (cmd == 'a') {
						...
						Serial.println(
							"Calibration aborted, the hard-iron bias won't be updated\r\n");
						STATE_AHRS = STATE_UKF_RUNNING;
					}
					...
					if (cmd == 'a') {
						...
						float m_tilt_x = Bx * cos(pitch) + By * sin(roll) * sin(pitch)
							+ Bz * cos(roll) * sin(pitch);
						...
						Serial.printf(
							"\nNorth identification finished, the north vector identified");
						PRINT_north();
						STATE_AHRS = STATE_UKF_RUNNING;
					}
