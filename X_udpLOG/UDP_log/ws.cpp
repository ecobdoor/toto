...
bool socketWS::isClient() {
	return _isClient;
}

void socketWS::setCaller(const string CALLER) {
	_caller = CALLER;
	while (_caller.length() < 24) {
		_caller += "~";
	}
}
string socketWS::readStr(const bool checkError) {
	char buf[DEF_UDP_DATA_BUFLEN];
	recvBuf((char *) &buf, DEF_UDP_DATA_BUFLEN, checkError);
//	U_PRINTF(" '%s'.\n", buf);
	return string(buf);
}
bool socketWS::writeStr(const string TXT, const bool checkError) {
	/*
	 Because we don't want a const char * in senBuf(...) for PackID tracing !!!
	 */
	char *cstr = new char[DEF_UDP_DATA_BUFLEN];
	strcpy(cstr, TXT.c_str());

	return sendBuf(cstr, 1 + TXT.length(), checkError); // +1 for final '\0' !!!
//	U_PRINTF(" '%s'.\n", TXT.c_str());
}
int socketWS::readInt() {
	char buf[DEF_UDP_DATA_BUFLEN];
	recvBuf((char *) &buf, DEF_UDP_DATA_BUFLEN);
	//U_PRINTF(" '%s'.\n", buf);
	return atoi(buf);
}
long int socketWS::readLongInt() {
	char buf[DEF_UDP_DATA_BUFLEN];
	recvBuf((char *) &buf, DEF_UDP_DATA_BUFLEN);
	//U_PRINTF(" '%s'.\n", buf);
	return atol(buf);
 ...
 */
socketWS::socketWS(const std::string NAME, int domain, int type, int protocol,
		uint16_t TIMEOUT) {
	_name = NAME + ":";
 ...
 */
void socketWS::connect2server(const string IP, const int PORT) {
// Filling server information
	...
	/* Si le client arrive à se connecter */
	        if(connect(_sockfd, (SOCKADDR*)&_REMOTEADDR, sizeof(_REMOTEADDR)) != SOCKET_ERROR)
	            printf("Connexion à %s sur le port %d\n", inet_ntoa(_REMOTEADDR.sin_addr), htons(_REMOTEADDR.sin_port));
	        else
...
bool socketWS::sendBuf(char * BUF, const uint16_t LEN, const bool checkError) {
	_cntWrite++;
	int n;
	bool ok = true;
	n = sendto(_sockfd, BUF, LEN, MSG_CONFIRM, (const struct sockaddr *) &_REMOTEADDR,
			sizeof(_REMOTEADDR));
	if (n != LEN) {
		if (n == -1) {
			ok = false;
...
int socketWS::recvBuf(char * BUF, const uint16_t BUFLEN, const bool checkError) {
	_cntRead++;
	int n;
	_len = sizeof(_REMOTEADDR);
	n = recvfrom(_sockfd, BUF, BUFLEN, MSG_WAITALL, (struct sockaddr *) &_REMOTEADDR,
			&_len);
	if (n == -1) {
		BUF[0] = '\0';
	}
...
void socketWS::setRecvBufSiz(const uint32_t Size) {
	size_t t1, t2;
	t1 = Size;
	t2 = sizeof(int);
	setsockopt(_sockfd, SOL_SOCKET, SO_RCVBUF, &t1, t2);
}
void socketWS::setSendBufSiz(const uint32_t Size) {
	size_t t1, t2;
	t1 = Size;
	t2 = sizeof(int);
...
void socketWS::setRecvTimeout(uint32_t TIMEOUT) {
	struct timeval tv;
	tv.tv_sec = 0;
	tv.tv_usec = 1000 * TIMEOUT;
	setsockopt(_sockfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
}
void socketWS::clear(const int TIMEOUT_MILLIS) {
	setRecvTimeout(0);
	_len = sizeof(_REMOTEADDR);
	bool done = false;
	while (!done) {
...
/*
 int socketWS::checkError(const string CHN, const int ERR, const int LVL) {
 socklen_t errcod;
